...
- GOT.37/47: command handlers register with `registerCommandHandler(type, handler)` in `apps/worker/src/runner/commands.ts`; a type with no handler is left unclaimed. The runner's `resume({ executionId, prompt, usageKind })` is the entry point for `resume_with_*` and `send_message`; it refuses when `canResume` is false (PR #31).
